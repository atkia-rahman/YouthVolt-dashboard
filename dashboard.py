...
# ══════════════════════════════════════════════════════════════
    ...
    if not sc_clean.empty and 'Notes' in sc_clean.columns:
        sc_anomalies = sc_clean[sc_clean['Notes'].notna() & (sc_clean['Notes'].astype(str).str.strip() != '') & (sc_clean['Notes'].astype(str).str.strip() != 'nan')][['Date','Class','Notes']].copy()
        sc_anomalies.columns = ['Date','Source','Note']
        anomalies.append(sc_anomalies)
    if not hh_clean.empty and 'Notes' in hh_clean.columns:
        hh_anomalies = hh_clean[hh_clean['Notes'].notna() & (hh_clean['Notes'].astype(str).str.strip() != '') & (hh_clean['Notes'].astype(str).str.strip() != 'nan')][['Date','Household ID','Notes']].copy()
